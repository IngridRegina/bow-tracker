...
Colour carries one thing, how many members: a single hue in five steps, light
to dark, checked for monotone lightness, visible gaps between steps, and a
light end that still clears 2:1 on the page so "one member" cannot be mistaken
for "none". The country list repeats every name and count in text, so nothing
depends on reading a shade.
